...
kissat_init (void)
{
  ...
  solver->unsatord_compacted = false;
...
void 
kissat_add_unsatord (kissat * solver, unsigned eidx) {
	assert(eidx < SIZE_STACK (solver->import));
	import *import = &PEEK_STACK (solver->import, eidx);
	
	if (import->imported) {
	  assert(IDX(import->lit) < VARS);
		int idx = IDX(import->lit);
		flags *f = FLAGS (idx);
		if(f->active && !f->eliminated && !f->fixed)
			add_node(solver, &solver->unsatord_list, idx);
	}
}
