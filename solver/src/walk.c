...
static void
import_decision_phases_neuroback (walker * walker)
{
  kissat *solver = walker->solver;
  INC (walk_decisions);
  ...
  value *const initial = solver->phases.initial; //INITIAL_PHASE;
  ...
  value = saved[idx];
      if (!value)
  value = initial[idx];//initial_phase;
      assert (value);
      if (saved[idx] != value)
  {
    saved[idx] = value;
...
init_walker (kissat * solver, walker * walker, litpairs * binaries)
{
  ...
  if(GET_OPTION (neural_backbone_initial) 
    || GET_OPTION(random_phase_initial)) {
    import_decision_phases_neuroback (walker);
  }
  else
