...
static inline void
add_unassigned_variable_back_to_queue (kissat * solver, links * links, queue* queue, unsigned lit)
{
  assert (!solver->stable);
  const unsigned idx = IDX (lit);
  if (links[idx].stamp > queue->search.stamp)
  ...
  if (solver->stable)
      ...
      for (const unsigned *p = q; p != old_end; p++)
	{
	  ...
	  if (level <= new_level)
	    ...
	    {
	      unassign (solver, values, lit);
	      add_unassigned_variable_back_to_queue (solver, links, &solver->queue, lit);
	      unassigned++;
	    }
	}
    }
