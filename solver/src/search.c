...
kissat_search (kissat * solver)
{
  ...
  if(GET_OPTION(neural_unsatord_stable_initial) 
    || GET_OPTION(neural_unsatord_stable_compact)) {
    solver->apply_unsatord_stable = true;
    solver->current_unsatord_node = solver->unsatord_list.head;
  }

  if(GET_OPTION(neural_unsatord_focused_initial) 
    || GET_OPTION(neural_unsatord_focused_compact)) {
    solver->apply_unsatord_focused = true;
    solver->current_unsatord_node = solver->unsatord_list.head;
  }
  ...
  while (!res)
    {
      ...
      if (conflict) {
        ...
        if(solver->apply_unsatord_stable && solver->stable) {
          solver->apply_unsatord_stable = false;
          solver->current_unsatord_node = solver->unsatord_list.head;
        }
        
        if(solver->apply_unsatord_focused && !solver->stable) {
          solver->apply_unsatord_focused = false;
          solver->current_unsatord_node = solver->unsatord_list.head;
        }
      }
