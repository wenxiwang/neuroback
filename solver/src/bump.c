...
static void
move_analyzed_variables_to_front_of_queue 
(kissat * solver, links* links, queue* queue)
{
...
// *INDENT-ON*
  ...
  for (all_stack (datarank, rank, solver->ranks))
    if (flags[idx = rank.data].active)
      kissat_move_to_front (solver, links, queue, idx);
...
kissat_bump_analyzed (kissat * solver)
{
  ...
  if (!solver->stable)
    move_analyzed_variables_to_front_of_queue 
      (solver, solver->links, &(solver->queue));
  else
